...
describe('LazyPipeline', () => {
  ...
  test('2. Should not evaluate the added stages before the terminal stage is triggered', () => {
    ...
    expect(mapConsumeSpy).not.toHaveBeenCalled();
    expect(distinctConsumeSpy).not.toHaveBeenCalled();
    expect(sortedConsumeSpy).not.toHaveBeenCalled();
    expect(limitConsumeSpy).not.toHaveBeenCalled();
  });
  ...
  test('4. Should throw an error if collect() is called again without calling resume() first', () => {
    const source = [2, 4, 5, 9, 6, 8, 10, 2, 4, 3];
    const pipeline = LazyPipeline.from(source);
  ...
  test('5. Should not throw an error when collect() is called after resume() had been called', () => {
    const source = [2, 4, 5, 9, 6, 8, 10, 2, 4, 3];
    const pipeline = LazyPipeline.from(source);
    const expected = Array.from(new Set(source))
      .map(e => e + 1)
