 ...
 * you'd like and reuse it in different places with no worries about data corruptions by calling {@link reset()}
    ...
    switch (event) {
      ...
      case PipelineEvent.DETACH_STAGE:
        if (stage instanceof IntermediateStage) {
          ...
          throw new Error(
            `[LazyPipeline] A non-intermediate stage was detached, detached staged was ${JSON.stringify(stage)}`
          );
        }
    }
  }

  collect<OUT>(collector: TerminalStage<IN, OUT>) {
    ...
    lastStage.pipeTo(collector);

    this._terminalStage = collector;

    collector.subscribe(this._watchForPipelineEvent);
